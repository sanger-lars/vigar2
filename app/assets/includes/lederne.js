...
class Person {
  constructor() {
    (this.navn = "ny leder"), (this.img = "mangler.jpg"), (this.alt = ""), (this.funktioner = "");
  }
}
...
function ret_leder(id) {
  ...
  nr = find_id(id);
  var L = Ledere[nr];
  m.html = `<img id="ret-billede" src="assets/images/ledere/${L.img}" alt="${L.alt}" >
...
function skriv_html() {
  ...
  for (var i = 0; i < Ledere.length; i++) {
    ...
    tekst += `<li class="column" draggable="true"><header><div>
      ...
      <button data-nr=${i} >RET</button> <button data-nr=${i}>SLET</button></div></header></li>
      `;
  }
  her.innerHTML = "";
  her.insertAdjacentHTML("beforeend", tekst);
  var navne = document.getElementById("columns").getElementsByTagName("button");
